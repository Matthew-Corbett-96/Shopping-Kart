...
const Item& Kart::get_item(int index) const
{
    ...
    if (shoping_list.empty()) throw std::length_error("Kart is Empty.\n");
    // if index is out of range then throw error
    if (index > list_length || index <= 0) throw std::range_error("Item Index is Out of Range.\n");
...
void Kart::set_name(const std::string& name)
{
    ...
    if (name.length() > 25) throw std::invalid_argument("Name of Kart Cannot be Greater Than 25 Characters.\n");
    this->kart_name = name;
}
...
void Kart::check_out(const float& distance) const
{
    ...
    if (distance < 0.00f) throw std::invalid_argument("Distance of Order Cannot Be Negative\n");
    ...
    for (auto& item : shoping_list) sum += item.get_price();
...
void Kart::print_kart() const
{
    ...
    for (auto& item : this->shoping_list)
        item.print_Item();
    Log
        "------------------------"  End;
}